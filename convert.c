...
printf("  Mantissa: 0x%-9x or: %6s", float_32.f_bits.mantissa, "");
for(int i = 12; i < 40; i++)
{
...
printf("  Exponent: 0x%-9x or: %1s", float_32.f_bits.exponent, "");
for(int i = 2; i < 11; i++)
{
	printf("%c", bit_string[i]);
}
